...
import { createClient, PADRAO, getLavanderia, PECAS, calcTotal } from '@/lib/supabase'
...
export default function LimpezasPage() {
  ...
  const byPredio = rows.reduce((acc, r) => {
    ...
    campos.forEach((c, i) => acc[r.predio].totais[i] += r[c])
    return acc
  ...
  async function handleSave() {
    ...
    // Delete existing for date and re-insert
    ...
    setSaving(false)
    if (err) { setError('Erro ao salvar: ' + err.message) } else { setSaved(true) }
  }
                  ...
                  return (
                    <tr key={pred} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2.5 text-center">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${d.lav === 'ELIS' ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700'}`}>
                          {d.lav}
                        </span>
                      </td>
                      <td className="px-3 py-2.5 text-center text-gray-700">{d.items.length}</td>
                      {d.totais.map((v: number, i: number) => (
                        <td key={i} className="px-2 py-2.5 text-center text-gray-700">{v}</td>
                      ))}
